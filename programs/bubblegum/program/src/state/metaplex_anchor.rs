...
pub struct MasterEdition(token_metadata::state::MasterEditionV2);

impl anchor_lang::AccountDeserialize for MasterEdition {
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> Result<Self> {
        try_from_slice_checked::<token_metadata::state::MasterEditionV2>(
            buf,
            token_metadata::state::Key::MasterEditionV2,
...
impl Deref for MasterEdition {
    type Target = token_metadata::state::MasterEditionV2;
...
#[derive(Clone, AnchorDeserialize, AnchorSerialize)]
pub struct TokenMetadata(token_metadata::state::Metadata);

impl anchor_lang::AccountDeserialize for TokenMetadata {
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> Result<Self> {
        try_from_slice_checked::<token_metadata::state::Metadata>(
            buf,
            token_metadata::state::Key::MetadataV1,
            MAX_METADATA_LEN,
        )
        .map(TokenMetadata)
        .map_err(Into::into)
...
impl Deref for TokenMetadata {
    type Target = token_metadata::state::Metadata;
